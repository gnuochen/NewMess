...
/**
 * Created by zouxiaobang on 15/10/21.
 */
public class RoundImageView extends ImageView{

    private int borderThickness = 0; // 边框宽度
    ...
    private void setCustomAttributes(AttributeSet attrs){
        ...
        borderOutsideColor = typedArray
                .getColor(R.styleable.roundedimageview_border_outside_color,
                        defaultColor);
        borderInsideColor = typedArray.getColor(
    ...
    protected void onDraw(Canvas canvas){
        ...
        int radius = 0;
        if (borderInsideColor != defaultColor
                && borderOutsideColor != defaultColor) {// 定义画两个边框，分别为外圆边框和内圆边框
            radius = (defaultWidth < defaultHeight ? defaultWidth
                    : defaultHeight) / 2 - 2 * borderThickness;
            // 画内圆
            drawCircleBorder(canvas, radius + borderThickness / 2,
                    borderInsideColor);
            // 画外圆
            drawCircleBorder(canvas, radius + borderThickness
                    + borderThickness / 2, borderOutsideColor);
        } else if (borderInsideColor != defaultColor
                && borderOutsideColor == defaultColor) {// 定义画一个边框
            radius = (defaultWidth < defaultHeight ? defaultWidth
